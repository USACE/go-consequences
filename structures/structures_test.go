...
func TestComputeConsequencesMultiHazard(t *testing.T) {
	...
	var d6 = hazards.ArrivalDepthandDurationEvent{}
	d6.SetDuration(0.0)
	d6.SetDepth(1.0)
	t6 := time.Date(1985, time.Month(2), 10, 0, 0, 0, 0, time.UTC)
	d6.SetArrivalTime(t6)

	var d7 = hazards.ArrivalDepthandDurationEvent{}
	d7.SetDuration(0.0)
	d7.SetDepth(0.0)
	t7 := time.Date(1985, time.Month(2), 15, 0, 0, 0, 0, time.UTC)
	d7.SetArrivalTime(t7)

	events := []hazards.ArrivalDepthandDurationEvent{d1, d2, d3, d4, d5, d6, d7}
	...
	et6 := time.Date(1985, time.Month(2), 20, 0, 0, 0, 0, time.UTC)
	et7 := time.Date(1985, time.Month(2), 21, 0, 0, 0, 0, time.UTC) // no reconstruction is calculated for a day when an event happens, so this event still pushes the completions data back by 1

	expectedResults := []time.Time{et1, et2, et3, et4, et5, et6, et7}
	expectedDmgs := []float64{10.0, 10.0, 9.5, 20.0, 18.0, 10.0, 0.0}
	expectedVals := []float64{90, 90, 85.5, 80, 72, 90, 95}
	...
	for i := range expectedResults {
		...
		if math.Abs(svalout.(float64)-float64(expectedVals[i])) > 0.000000001 {
			t.Errorf("Times Rebuilt was %3.2f. Expected: %3.2f.\n", svalout, expectedVals[i])
		}
	}

	// Check final results
	eFinalVal := 95.0
	gFinalVal, err := results.Fetch("final structure value")
	if err != nil {
		panic(err)
	}
	var eTimesRebuilt int32 = 6
	gTimesRebuilt, err := results.Fetch("times rebuilt")
	if err != nil {
		panic(err)
	}

	if math.Abs(eFinalVal-gFinalVal.(float64)) > 0.000000001 {
		t.Errorf("Final Structure Value was %3.2f. Expected: %3.2f.\n", gFinalVal, eFinalVal)
	}

	if eTimesRebuilt != gTimesRebuilt.(int32) {
		t.Errorf("Final Structure Value was %d. Expected: %d.\n", gTimesRebuilt, eTimesRebuilt)
	}
}
