...
func Test_StreamProcessor(t *testing.T) {
	...
	rw, _ := resultswriters.InitGpkResultsWriter_Projected("/workspaces/Go_Consequences/data/test2016.gpkg", "agdamage", 5070) // testing data output
	defer rw.Close()
...
func Test_StreamAbstract(t *testing.T) {
	...
	rw, _ := resultswriters.InitGpkResultsWriter_Projected("/workspaces/Go_Consequences/data/abstract.gpkg", "agdamage", 5070) // testing data output
	defer rw.Close()
	at := time.Date(1984, time.Month(4), 15, 0, 0, 0, 0, time.UTC)
