...
func InitGeoparquetResultsWriter_Projected(filepath string, layerName string, ESPG int) (*geoparquetResultsWriter, error) {
	driverOut := gdal.OGRDriverByName("Parquet")
	dsOut, okOut := driverOut.Create(filepath, []string{})
	if !okOut {
		//error out?
		return &geoparquetResultsWriter{}, errors.New("geoparquet at path " + filepath + " not created")
	}
	...
	return &geoparquetResultsWriter{FilePath: filepath, LayerName: layerName, ds: &dsOut, Layer: &newLayer, index: 0}, nil
}
func InitGeoparquetResultsWriter(filepath string, layerName string) (*geoparquetResultsWriter, error) {
	return InitGeoparquetResultsWriter_Projected(filepath, layerName, 4326)
}
